...
// Rota para listar álbuns por gênero (pop, rock, metal, alternativo)
router.get("/albuns/:genero", (req, res) => {
  const generoParam = req.params.genero.toLowerCase();
  let generoBusca = null;

  // mapear a rota amigável para o gênero real presente nos dados
  switch (generoParam) {
    case "pop":
      generoBusca = "Pop";
      break;
    ...
    case "metal":
      ...
      break;
    case "alternativo":
      generoBusca = "Rock Alternativo";
      break;
    default:
      // se não reconhecido, mostra todos
      return res.render("albuns", { albuns });
  }

  const filtrados = getAlbunsPorGenero(generoBusca);
  res.render("albuns", { albuns: filtrados });
});
