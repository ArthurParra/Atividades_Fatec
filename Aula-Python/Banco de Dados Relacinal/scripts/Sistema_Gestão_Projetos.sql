# Exercício - Sistema de Gestão de Projetos
create database gestao_projetos_ArthurParra;
use gestao_projetos_ArthurParra;
...
create table Alocacoes(
    ...
    id_projeto int,
    id_papel int,
    constraint fk_alocacao_pessoa foreign key (id_pessoa) references Pessoas(id_pessoa),
    constraint fk_alocacao_projeto foreign key (id_projeto) references Projetos(id_projeto),
    constraint fk_alocacao_papel foreign key (id_papel) references Papeis(id_papel)
);
 
create table Tarefas(
	id_tarefa int primary key auto_increment,
    id_responsavel int,
    constraint fk_tarefa_pessoa foreign key (id_responsavel) references Pessoas(id_pessoa),
    id_projeto int,
    constraint fk_tarefa_projeto foreign key (id_projeto) references Projetos(id_projeto),
    titulo varchar(50),
    descricao varchar(200),
    status varchar(50)
);
...
insert into alocacoes (id_pessoa, id_projeto, id_papel) values
...
insert into tarefas (id_responsavel, id_projeto, titulo, descricao, status) values
...
# 06
select id_tarefa, titulo, status from tarefas where id_projeto = 1 order by status;
select id_tarefa, titulo, status from tarefas where id_projeto = 1 order by titulo;
# 07
select id_tarefa, titulo, status from tarefas where status = "concluída" and id_projeto = 3 order by id_tarefa desc;
# 08
select id_tarefa, titulo, status from tarefas where status != "concluída" and id_projeto = 4 order by titulo;
# 09
select id_tarefa, id_projeto, titulo, status from tarefas where titulo like "%cadastro%" order by id_projeto; 
select id_tarefa, id_projeto, titulo, status from tarefas where titulo like "%cadastro%" order by titulo; 
# 10
select id_tarefa, titulo, status from tarefas where id_projeto = 2 limit 2 offset 2;